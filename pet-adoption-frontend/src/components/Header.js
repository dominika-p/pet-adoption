	...
	return (
		<header className='header'>
			<div className='header-container'>
				...
				<div ref={hamburgerRef} className={`hamburger ${isOpen ? 'open' : ''}`} onClick={() => setIsOpen(!isOpen)}>
					<div></div>
					<div></div>
					<div></div>
				</div>
					...
					<div className='account-mobile'>
						{!currentUser ? (
							...
							<>
								...
								<button className='logout-btn' onClick={handleVolunteerLogout}>
									Wyloguj
								</button>
							</>
						)}
				...
				<div className='account-desktop'>
					{!currentUser ? (
						<Link to='/volunteer-login' className='account-link'>
							Konto
						</Link>
					) : isAdmin ? (
						<>
						...
						<>
							<Link to='/volunteer-dashboard' className='account-link'>
								Moje konto
							</Link>
							<button onClick={handleVolunteerLogout} className='logout-btn'>
								Wyloguj
							</button>
						</>
					)}
