...
const AdminMessages = () => {
	...
	const sendReply = (id, replyText) => {
		...
		fetch(`http://localhost:5000/api/contact/${id}/reply`, {
			...
			.then(updatedMsg => {
				setMessages(prev => prev.map(msg => (msg.id === id ? { ...updatedMsg, replyText: '' } : msg)))
			})
			.catch(err => console.error(err))
	}

	const sortedMessages = [...messages].sort((a, b) => (!!a.reply === !!b.reply ? 0 : !!a.reply ? 1 : -1))
						...
						{!msg.reply ? (
							<>
								<textarea
									...
									value={msg.replyText}
									onChange={e =>
										setMessages(prev => prev.map(m => (m.id === msg.id ? { ...m, replyText: e.target.value } : m)))
									}
								/>
							...
							<div className='answered-info'>
								<p>
									<strong>Odpowiedź wysłana:</strong> {msg.reply}
								</p>
								<button onClick={() => deleteMessage(msg.id)}>Usuń</button>
							</div>
						)}
